...
indices_dinamica = st.sidebar.multiselect(label='Selecione os índices:', options=colunas_analise)
colunas_analise_filtradas = [c for c in colunas_analise if c not in indices_dinamica] 
colunas_dinamica = st.sidebar.multiselect(label='Selecione as colunas:', options=colunas_analise_filtradas)

valor_analise = st.sidebar.selectbox(label='Selecione o valor da análise:', options=list(colunas_numericas.keys()))

funcoes_agregacao ={'soma':'sum', 'contagem':'count'}
metrica_analise = st.sidebar.selectbox(label='Selecione a métrica da análise:', options=list(funcoes_agregacao.keys()))
